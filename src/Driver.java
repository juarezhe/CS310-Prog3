...
//////////////////////////////////////////////////////////////////////////////       
		...
		try {
			...
			if (!pq.delete(50) || !pq2.delete(new PrioritizedItem(50, 9)))
				throw new RuntimeException("INSERTED 50 BUT CAN'T DELETE 50");
					...
					|| pq2.contains(new PrioritizedItem(50, 9)))
				throw new RuntimeException("ALREADY DELETED 50 AND YET CAN STILL DELETE ANOTHER 50(s)");
			System.out.println("Done!");
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				if (i == size / 2)
					...
					pq2.insert(new PrioritizedItem(2, (int) (size * Math.random())));
				pq2.remove();
				if (pq.size() != 0 || pq2.size() != 0)
					throw new RuntimeException("REMOVED ALL BUT SIZE IS NON-ZERO");
			}
	...
	private void test10() {
		try {
			...
			if (pq2.remove().compareTo(new PrioritizedItem(3, 0)) > 0 || pq2.remove().compareTo(pq2.remove()) > 0
					|| pq.remove() - pq.remove() > 0 || pq.remove() != 3)
				throw new RuntimeException("pq HAS OUT OF ORDER REMOVAL");
	...
	private void test12() {
		try {
			...
			if (pq.size() != 1 || pq.peek() == null || !pq.contains(pq.peek()) || pq.remove() == null || pq2.size() != 1
					|| pq2.peek() == null || !pq2.contains(pq2.peek()) || pq2.remove() == null)
